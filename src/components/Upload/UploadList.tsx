    ...
    return (
        <>
                <ul className="instant-upload-list">
                    {fileList.map((file) => ( // 遍历文件列表 ：
                    <li key={file.uid}  className="instant-upload-list-item">
                     <div style={{flex: 1}}>
                        <Icon icon="file" style={{marginRight: '0.5rem'}}></Icon>
                         {file.name}
                     </div>
                     <span className="instant-upload-list-item-status">
                        {file.status === 'uploading' && <Icon icon="spinner" spin></Icon>}
                        {file.status === 'success' && <Icon icon="check-circle" theme="success"></Icon>}
                        {file.status === 'error' && <Icon icon="times-circle" theme="danger"></Icon>}
                     </span>
                      <span className="instant-upload-list-item-remove">
                        <Icon icon="times-circle"
                        onClick={() => handleRemove(file)} ></Icon>
                        {/*注意易错处  
